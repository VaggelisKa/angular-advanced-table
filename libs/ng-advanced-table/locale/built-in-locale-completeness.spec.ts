...
describe('built-in companion UI locale completeness', () => {
  it('ships required consumer-facing labels and formatter hooks in every built-in UI locale', () => {
    ...
    for (const [localeId, locale] of localeEntries) {
      ...
      expectNonEmptyText(
        locale.pager?.accessibilityLabels?.previousPageAriaLabel,
        `${localeId}: pager.accessibilityLabels.previousPageAriaLabel`
      );
      expectNonEmptyText(
        locale.pager?.accessibilityLabels?.nextPageAriaLabel,
        `${localeId}: pager.accessibilityLabels.nextPageAriaLabel`
      );
      expectFormattedText(
      ...
      expectNonEmptyText(
        locale.selection?.accessibilityLabels?.selectAllAriaLabel,
        `${localeId}: selection.accessibilityLabels.selectAllAriaLabel`
      );
      expectFormattedText(
