...
- Keyboard accessibility overhaul across the table header, toolbar items, pagination, and pinned zones. ([#347](https://github.com/VaggelisKa/angular-advanced-table/pull/347))
...
- Move Nx off the `23.1.0-beta.0` prerelease onto `23.1.1` stable. ([#337](https://github.com/VaggelisKa/angular-advanced-table/pull/337))
...
- Refresh the workspace dependency overrides so security patches can reach the tree again. ([#335](https://github.com/VaggelisKa/angular-advanced-table/pull/335))
...
- Bound public table-state comparisons and global-filter traversal so cyclic, deeply nested, or excessively broad consumer values terminate safely instead of exhausting the call stack or CPU. ([#340](https://github.com/VaggelisKa/angular-advanced-table/pull/340))
...
- Document maintainer guidance learned from recent merged work: nightly snapshots must derive their prerelease base from the highest pending version-plan bump without consuming release plans, MCP request/response bounds must stay in place, virtualization remains an engine-owned optional entry-point behavior, remote windowing keeps one loaded window over logical coordinates, and consumer-controlled table value traversals must remain bounded and cycle-safe. ([#344](https://github.com/VaggelisKa/angular-advanced-table/pull/344))
- Cut stable releases on a weekly schedule instead of only on demand. ([#346](https://github.com/VaggelisKa/angular-advanced-table/pull/346))
...
- Move the workspace onto Angular 22.1: framework and components to 22.1.1, the CLI packages (`@angular/build`, `@angular/cli`, `@angular/ssr`) to 22.1.3, and `ng-packagr` to 22.1.1. ([#336](https://github.com/VaggelisKa/angular-advanced-table/pull/336))
...
- Document that bounded structural table-state comparisons conservatively report a change when extremely deep or broad consumer values exhaust the comparison budget. ([#342](https://github.com/VaggelisKa/angular-advanced-table/pull/342))
...
- Reorganize the core `ng-advanced-table` entry into flat capability folders directly under `src/` — `hotkey-a11y/`, `cell-interaction/`, `reorder/`, `resize/`, and the `table/` component shell. Only nested element folders such as `common/` and `utils/` are boundary-typed by their deepest folder; wrapper roots such as `resize/`, `reorder/`, and `table/` stay unmatched. The shared `common`/`utils`/`ui`/`domain-logic` layers and the per-instance state hub (`src/domain-logic/table.state.ts` plus the TanStack table instance) stay core. Internal refactor and file layout only — no public API, `exports`, or runtime behavior change; the published bundles are unchanged. ([#224](https://github.com/VaggelisKa/angular-advanced-table/pull/224))
