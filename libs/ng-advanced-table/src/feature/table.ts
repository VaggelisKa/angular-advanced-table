...
import { handleCellInteractionFocusIn, handleCellInteractionKeydown } from '../utils/cell-interaction';
import { isSpaceShortcutKey, validateKeybindings } from '../utils/keybindings';
import {
  getColumnDefLeafIds,
...
export class NatTable<TData extends RowData = RowData> implements NatTableUiController<TData> {
   ...
   */
  private resizeCommit: { readonly columnId: string; readonly width: number } | null = null;
  ...
  protected readonly columnResizeGuide = computed<{ readonly left: number; readonly offset: number } | null>(() => {
  ...
  private getResizeBounds(column: Column<TData, unknown>): { readonly min: number; readonly max: number | null } {
   ...
   */
  private getResizeFitBounds(column: Column<TData, unknown>): { readonly min: number; readonly max: number | null } {
