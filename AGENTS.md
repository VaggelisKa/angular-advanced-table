...
- Keep pure table-state and column helper functions as focused `*.util.ts` modules — shared ones under `libs/ng-advanced-table/src/utils/` (e.g. `column-def.util.ts`, `sorting.util.ts`) and feature-specific ones under `src/feature/<name>/utils/` (e.g. `feature/resize/utils/column-resize.util.ts`); keep `NatTableState` (`src/domain-logic/table.state.ts`) responsible for state ownership, TanStack wiring, and derived computeds; keep `NatTable` (`src/feature/table.ts`) focused on input/output bridging, template refs, and DOM-coupled handlers. Keep a11y and header-measurement behavior in dedicated `src/domain-logic/` services, and keep feature-owned behavior in its feature folder's `domain-logic/` (resize in `src/feature/resize/domain-logic/table-resize.service.ts`, reorder in `src/feature/reorder/domain-logic/table-reorder.service.ts`).
...
- When changing table keybindings, update the `src/feature/hotkey-a11y/` files together — `common/keybindings.type.ts` and `common/keybindings.const.ts`, `utils/keybindings.util.ts` and `utils/shortcut-parsing.util.ts`, `domain-logic/keybindings.provider.ts`, the `NatTableHotkeyA11y` directive (`hotkey-a11y.directive.ts`), and `keybindings.md` — along with public API exports and table/surface input coverage, so configured shortcuts, conflict warnings, and `aria-keyshortcuts` stay aligned.
...
- Showcase route and docs discovery are generated for agents: after changing canonical doc/example routes or discovery metadata, run the showcase `generate-discovery` target and commit regenerated `public/robots.txt`, `public/sitemap.xml`, `public/.well-known/api-catalog`, and `public/.well-known/mcp/server-card.json`. Do not commit the ignored `api/markdown-pages.generated.json` build artifact.
