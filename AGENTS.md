...
- Keep `@tanstack/angular-table` as an installed runtime dependency of `ng-advanced-table` and the only `allowedNonPeerDependencies` entry in `ng-package.json`; keep Angular framework, Aria, and CDK packages as required peers. Consumer-facing docs/snippets should import forwarded authoring types and helpers such as `ColumnDef`, state slice types, and `flexRenderComponent` from `ng-advanced-table`, not directly from `@tanstack/angular-table`.
