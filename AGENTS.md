...
Inside each entry point, source is organized into element folders enforced by `eslint-plugin-boundaries`. The element-layer settings (`boundaries/elements` + `boundaries/dependencies`) live in the shared `lint-suite` package (`node_modules/lint-suite/eslint.js`); each project's `eslint.config.mjs` (e.g. `libs/ng-advanced-table/eslint.config.mjs`) only re-imports and spreads that shared `boundaries` config, so nesting new element folders enforces boundaries with **no per-project lint-config change**. A file is typed by its **deepest** element folder — a folder whose name matches `common`/`utils`/`domain-logic`/`ui`/`data-access`/`feature` — so `components/feature/<comp>/` is `feature` and `components/ui/<comp>/` is `ui`; a core feature nested as `src/feature/<name>/utils/` or `src/feature/<name>/domain-logic/` takes that inner `utils`/`domain-logic` type, while `src/feature/<name>.ts` and the `src/feature/table.ts` shell type as `feature`. The entry-point folder name (`components`, `render-metrics`) and a non-element wrapper folder (e.g. `resize`, `hotkey-a11y`) never decide the element. Allowed direction (a layer may import itself and everything below it):
...
Within the core `ng-advanced-table` entry, leaf capabilities are grouped as feature folders under `src/feature/` — `hotkey-a11y/`, `cell-interaction/`, `reorder/`, and `resize/` — each nesting its own `common/`, `utils/`, and/or `domain-logic/` so its consts, pure helpers, and services are boundary-typed by the deepest folder (no lint-config change needed). The shared `src/common/`, `src/utils/`, `src/ui/`, and `src/domain-logic/` layers stay flat and core, as does the state hub `src/domain-logic/table.state.ts` and the sibling services `table.service.ts`, `table-a11y.service.ts`, and `table-header-measurement.service.ts`. `hotkey-a11y`, `reorder`, and `resize` expose their public symbols through a curated `feature/<name>/index.ts` composed into `src/feature/index.ts`; `cell-interaction` has no barrel because its `NatTableCell` directive is internal, imported directly by the `src/feature/table.ts` shell. There is no `src/utils/index.ts` — no public helpers remain, so `src/index.ts` re-exports `common`, `domain-logic`, `feature`, `ui`, and the forwarded `@tanstack/angular-table` surface only.
