...
| `cellMaxLines`      | `number \| 'none'`                                                        | Maximum body-cell content lines before truncation; defaults to `2`             |
...
Body cell content is clamped to two lines by default. Use `cellHeight` to make a column's body cells fixed-height, set `cellMaxLines` to a different line count, or set `cellMaxLines: 'none'` for columns with custom interactive renderers that should not be line-clamped.
