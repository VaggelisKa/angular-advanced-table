...
import { PaginationStickyAlt } from './pagination-sticky-alt';

describe('PaginationStickyAlt', () => {
  let fixture: ComponentFixture<PaginationStickyAlt>;
  ...
  beforeEach(async () => {
    ...
    await TestBed.configureTestingModule({
      imports: [PaginationStickyAlt],
      providers: [provideZonelessChangeDetection()]
    }).compileComponents();

    fixture = TestBed.createComponent(PaginationStickyAlt);
  ...
  it('should render a right-pinned three-dots actions menu for each row', async () => {
    ...
    expect(actionTriggers).toHaveLength(50);

    const firstTrigger = actionTriggers[0];

    expect(firstTrigger.getAttribute('aria-label')).toBe('Open demo actions for ord-10000');
  ...
  it('should sort the mock rows from a header action', () => {
    ...
    expect(firstRowHeader.textContent.trim()).toBe('ORD-10001');
  });
