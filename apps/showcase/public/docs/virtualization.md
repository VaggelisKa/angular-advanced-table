...
**Placeholder rows are real grid rows.** Each carries its absolute `aria-rowindex`, `aria-busy="true"`, and one structurally correct cell per visible column, so a screen reader is never told there is data in the gap; the built-in copy reads "Row 1,000,001 of 2,000,000 is loading." and localizes through the `accessibility.placeholderRow` formatter in `ng-advanced-table/locale`. The optional `natTableRowPlaceholder` template renders inside every placeholder cell with the logical row index and the column; without it the cells stay empty at the fixed row height. Placeholder cells are ordinary roving-tabstop grid cells: arrow, page, and Control/Command + Home/End navigation move through them exactly as through data cells, and a placeholder cell that holds focus when its window arrives hands focus to the fetched row's cell in the same column.
