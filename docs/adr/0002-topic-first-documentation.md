...
The target Documentation Groups are Start, Core Model, Capabilities, Accessibility and UX, and Advanced. The Example Gallery keeps only the multiple-features demo and table builder. Removed one-feature example routes are not kept as compatibility redirects; those demos move into their relevant Documentation Topics.
