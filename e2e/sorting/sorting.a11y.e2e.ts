...
test('renders sorting grids and handles programmatic single-column sort actions via keyboard', async ({
  page,
}) => {
  ...
  const clearBtn = page.getByRole('button', { name: 'Clear Sorting' }).first();
...
test('handles programmatic multi-column sorting via keyboard', async ({ page }) => {
  ...
  const clearBtn = page.getByRole('button', { name: 'Clear Sorting' }).last();
  await clearBtn.focus();
  await page.keyboard.press('Space');
  await expect(multiStateTag).toContainText('None');
});
